    ...
    <tr className="text-black border-b-2">
      ...
      <td>
        <button
          ...
          disabled={role === "admin"}
          className={`${
            role === "admin" ? "opacity-25 cursor-not-allowed" : ""
          }  px-6 py-2 bg-[#1b1e34] text-white rounded m-1 text-sm block mx-auto`}
        >
          Make Admin
        </button>
        <button
          onClick={() => updateUserRole("instructor", _id)}
          disabled={role === "instructor"}
          className={`${
            role === "instructor" ? "opacity-25 cursor-not-allowed" : ""
          }  px-6 py-2 rounded m-1 text-sm bg-[#1b1e34] text-white block mx-auto`}
        >
          Make Instructor
        </button>
      </td>
